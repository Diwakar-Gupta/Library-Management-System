...
class DummyDataMixin(object):
    ...
    def setUpTestData(cls):
        ...
        abrar.save()
        Account.objects.create(id=30,user=abrar, status=AccountStatus.Active, phone_number='+91898989899')
        ...
        book = Book.objects.create(isbn='453678754', title='Hands on machine learining', subject='Data Science', publisher='Oreally', language='Englist', numer_of_pages=899)
        BookItem.objects.create(book=book, barcode='barcode123', is_reference_only=False, price=100, date_of_purchase=datetime.now().date(), placed_at=Rack.objects.create(number=9, location_identifier='RL'))
        BookItem.objects.create(book=book, barcode='barcode123-1', is_reference_only=True, price=100, date_of_purchase=datetime.now().date(), placed_at=Rack.objects.create(number=9, location_identifier='RQ'))
